...
public class ClienteDetailFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView = inflater.inflate(R.layout.fragment_cliente_detail,
				container, false);

		nomeText_ = ((EditText) rootView.findViewById(R.id.edit_text_nome));
		cognomeText_ = ((EditText) rootView
				.findViewById(R.id.edit_text_cognome));
			...
			public void onClick(View v) {
				...
				if (parent != null) {
					if (isInputEnabled_) {
						...
						boolean noEmpty = chechEmptyFields(nome, cognome,
								numero, fotoVisoAcquisita_,
								fotoDocumentoAcquisita_);

						AlertDialog.Builder esitoDialog = new AlertDialog.Builder(
								getActivity());
						String positiveButtonTitle = getResources().getString(
								R.string.button_ok);
						esitoDialog.setPositiveButton(positiveButtonTitle,
						...
						if (noEmpty) {
							...
							boolean createUserFileOk = SDCard.updateUser(
									cliente_, newUser);
							boolean moveImagesOk = SDCard
									.moveTempImages(numero);
							if (createUserFileOk && moveImagesOk) {
								esitoDialog.setMessage(getResources()
										.getString(R.string.dialog_ok_text));
								mDettaglioCallbacks.onStateChanged();
							} else {
								esitoDialog.setMessage(getResources()
										.getString(R.string.dialog_ko_text));
							}
						} else {
							esitoDialog.setTitle(getResources().getString(
									R.string.dialog_vuoti_title));
							esitoDialog.setMessage(getResources().getString(
									R.string.dialog_vuoti_text));
						}
						esitoDialog.show();
					} else {
		...
		salvaButton_.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (parent != null) {
					...
					boolean noEmpty = chechEmptyFields(nome, cognome, numero,
							fotoVisoAcquisita_, fotoDocumentoAcquisita_);
					...
					if (noEmpty) {
						...
						if (createUserFileOk && moveImagesOk) {
							esitoDialog.setMessage(getResources().getString(
									R.string.dialog_ok_text));
							mDettaglioCallbacks.onStateChanged();
						} else {
							esitoDialog.setMessage(getResources().getString(
									R.string.dialog_ko_text));
							mDettaglioCallbacks.onStateChanged();
						}
					} else {
						esitoDialog.setTitle(getResources().getString(
		...
		fotoVisoButton_ = ((Button) rootView
				.findViewById(R.id.button_foto_viso));
		fotoVisoButton_.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				String root = Environment.getExternalStorageDirectory()
						.toString() + SDCard.APPFOLDER;
		...
		fotoDocumentoButton_ = ((Button) rootView
				.findViewById(R.id.button_foto_documento));
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			...
			bitmapDocumento_ = savedInstanceState
					.getParcelable(DOCUMENTO_BITMAP);
			setupImagePreview(VISO_BITMAP);
			setupImagePreview(DOCUMENTO_BITMAP);

		} else if (cliente_ != null) {

			dialogAttesa_ = ProgressDialog
					.show(getActivity(), null, null, true);
			loadingDocumento_ = loadingViso_ = true;
			new LoadImageFromGallery().execute(VISO_BITMAP);

			new LoadImageFromGallery().execute(DOCUMENTO_BITMAP);
		}
	}
			...
			if (code > 0) {
				...
				default:
					Toast.makeText(getActivity(),
							"Errore acquisizione immagine.", Toast.LENGTH_SHORT)
							.show();
					break;
				}
			} else {
	...
	private void saveCapturedImage(Intent data, int imageType) {
		if (data != null) {
			...
			if (imageType == FOTO_VISO_REQUEST) {
				// TODO
				bitmapViso_ = bitmap;
				imagePathName = root + SDCard.TEMP_IMG_PATH
						+ SDCard.TEMP_IMG_VISO;
				imageView = (ImageView) getActivity().findViewById(
						R.id.imageview_foto_viso);
				fotoVisoButton_.setText(getResources().getString(
						R.string.button_foto_modifica));
			} else if (imageType == FOTO_DOCUMENTO_REQUEST) {
				// TODO
				bitmapDocumento_ = bitmap;
				imagePathName = root + SDCard.TEMP_IMG_PATH
						+ SDCard.TEMP_IMG_DOCUMENTO;
				imageView = (ImageView) getActivity().findViewById(
						R.id.imageview_foto_documento);
				fotoDocumentoButton_.setText(getResources().getString(
						R.string.button_foto_modifica));
			}
		...
		if (data != null) {
			...
			if (data != null && data.getData() != null) {
				...
				if (cursor != null) {
					...
					imageView.setVisibility(View.VISIBLE);
					saveTempImage(tempPath, imagePathName,
	...
	private void saveTempImage(String tempPath, String imagePathName,
			Bitmap bitmap, boolean isSalvataggioViso) {
	...
	private void startDialog(final int requestCode) {
		...
		String positiveButtonTitle = getResources().getString(
				R.string.button_galleria);
		myAlertDialog.setPositiveButton(positiveButtonTitle,
				new DialogInterface.OnClickListener() {
		...
		String negativeButtonTitle = getResources().getString(
				R.string.button_camera);
		myAlertDialog.setNegativeButton(negativeButtonTitle,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface arg0, int arg1) {
						Intent pictureActionIntent = new Intent(
	...
	private boolean chechEmptyFields(String nome, String cognome,
			String numero, boolean fotoViso, boolean fotoDocumento) {
	...
	private class LoadImageFromGallery extends AsyncTask<String, Void, String> {
		@Override
		protected String doInBackground(String... params) {
			...
			if (imageType == DOCUMENTO_BITMAP) {
				bitmapDocumento_ = SDCard.getPreviewImage(cliente_,
						SDCard.DOCUMENTO);
			}
	...
	private void setThumbImageView(ImageView imageview, Bitmap bitmap) {
		...
		imageview.setImageBitmap(BitmapFactory.decodeByteArray(array, 0,
				array.length));
